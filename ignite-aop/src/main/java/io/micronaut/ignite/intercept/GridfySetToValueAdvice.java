...
public class GridfySetToValueAdvice extends GridifySetToValueAbstractAspect implements MethodInterceptor<Object, Object> {
    private final BeanContext beanContext;

    public GridfySetToValueAdvice(BeanContext beanContext) {
        this.beanContext = beanContext;
    }

    @Override
    public Object intercept(MethodInvocationContext<Object, Object> context) {
        Optional<AnnotationValue<GridifySetToValue>> opt = context.findAnnotation(GridifySetToValue.class);
        if (!opt.isPresent()) {
        ...
        if (!gridifyInterceptorClazz.equals(GridifyInterceptor.class)) {
            ...
            try {
                // need to avoid reflection
                if (!interceptor.isGridify(mtd.getAnnotation(GridifySetToValue.class), arg)) {
                    return context.proceed();
                }
        ...
        try {
            ...
            return execute(mtd, ignite.compute(), context.getDeclaringType(), arg, nodeFilter, threshold, splitSize, timeout);
        } catch (IgniteCheckedException e) {
